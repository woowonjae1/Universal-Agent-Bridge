...
The OpenClaw adapter accepts both the current Gateway shape (`message`) and older/simple aliases (`text`, `prompt`, `input`, `content`) for `agent`, `agent.stream`, `chat.send`, and `chat.stream`. It normalizes those aliases before calling OpenClaw, fills `sessionKey` from the bridge session id or `default` when omitted, and fills `idempotencyKey` from the bridge request id.
